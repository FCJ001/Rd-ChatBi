...
#
#   1. PostgreSQL  → bi_datasources 注册 + nl2sql_tables / columns / metrics /
#                    nl2sql_column_metrics（带 datasource_id，按数据源隔离）
#   2. Milvus      → chatbi_{prefix}_columns / chatbi_{prefix}_metrics collection
#   3. Elasticsearch → chatbi_{prefix}_values index（仅 sync: true 的列）
#
...
from src.nl2sql.models.datasource import BiDatasource

# ── 存储命名（运行时按数据源 prefix 重设）────────────────────────

COLUMN_COLLECTION = "chatbi_columns"
METRIC_COLLECTION = "chatbi_metrics"
VECTOR_DIM = 1024  # text-embedding-v3 输出维度

VALUE_INDEX = "chatbi_values"
VALUE_MAPPING = {
    "properties": {
        "id": {"type": "keyword"},
...
async def main(datasource: str | None = None, dry_run: bool = False, rebuild: bool = False):
    ...
    if meta.datasource is None:
        raise SystemExit(f"{yaml_path} 缺少 datasource 段（多数据源模式必须配置）")
    ds = meta.datasource
    ...
    # 按数据源前缀重设 Milvus/ES 存储命名（物理隔离）
    global COLUMN_COLLECTION, METRIC_COLLECTION, VALUE_INDEX
    COLUMN_COLLECTION = f"chatbi_{ds.milvus_prefix}_columns"
    METRIC_COLLECTION = f"chatbi_{ds.milvus_prefix}_metrics"
    VALUE_INDEX = f"chatbi_{ds.es_prefix}_values"

    mode = "全量重建" if rebuild else "增量更新"
    print(f"数据源: {code}（{ds.name}）")
    print(f"模式: {mode}")
    print(f"  Milvus: {COLUMN_COLLECTION} / {METRIC_COLLECTION}")
    print(f"  ES: {VALUE_INDEX}")
    ...
    async with pg_factory() as db:
        if dry_run:
            ...
            ds_id = await _upsert_datasource(db, ds)
            if rebuild:
                await _rebuild_pg(db, meta, ds_id)
            else:
                await _upsert_pg(db, meta, ds_id)
...
async def _upsert_datasource(db: AsyncSession, ds) -> int:
    """按 code upsert bi_datasources 注册行，返回 datasource_id"""
    row = (await db.execute(
        select(BiDatasource).where(BiDatasource.code == ds.code)
    )).scalar_one_or_none()
    if row:
        row.name = ds.name
        row.dsn = ds.dsn
        ...
        db.add(row)
        await db.flush()
    print(f"  数据源注册: {ds.code} (id={row.id})")
    await db.commit()
    return row.id
...
async def _rebuild_pg(db: AsyncSession, meta: MetaConfig, ds_id: int):
    """删除本数据源旧数据后重新写入（其他数据源不受影响）"""
    await db.execute(delete(ColumnMetricModel).where(ColumnMetricModel.datasource_id == ds_id))
    await db.execute(delete(Nl2sqlMetric).where(Nl2sqlMetric.datasource_id == ds_id))
    await db.execute(delete(Nl2sqlColumn).where(Nl2sqlColumn.datasource_id == ds_id))
    await db.execute(delete(Nl2sqlTable).where(Nl2sqlTable.datasource_id == ds_id))

    table_map: dict[str, int] = {}
    for t in meta.tables:
        row = Nl2sqlTable(datasource_id=ds_id, table_name=t.name, role=t.role, description=t.description)
        db.add(row)
        await db.flush()
    ...
    for m in meta.metrics:
        for col_ref in m.relevant_columns:
            ...
            row = ColumnMetricModel(datasource_id=ds_id, column_id=col_ref, metric_id=metric_id)
            db.add(row)
...
async def _upsert_pg(db: AsyncSession, meta: MetaConfig, ds_id: int):
    """按唯一键对比 YAML 与 DB，增/改/删（仅限本数据源范围）"""
    # ── 表 ──
    existing_tables = {
        t.table_name: t
        for t in (await db.execute(
            select(Nl2sqlTable).where(Nl2sqlTable.datasource_id == ds_id)
        )).scalars()
    }
            ...
            for cname in col_ids:
                ...
                await db.execute(
                    delete(ColumnMetricModel).where(
                        ColumnMetricModel.datasource_id == ds_id,
    ...
    for t_id in table_map.values():
        for c in (
            await db.execute(
                select(Nl2sqlColumn).where(
                    Nl2sqlColumn.table_id == t_id,
                    Nl2sqlColumn.datasource_id == ds_id,
                )
            )
        ).scalars():
    ...
    existing_metrics = {
        ...
        for m in (await db.execute(
            select(Nl2sqlMetric).where(Nl2sqlMetric.datasource_id == ds_id)
        )).scalars()
    }
...
if __name__ == "__main__":
    ...
    asyncio.run(main(datasource=args.datasource, dry_run=args.dry_run, rebuild=args.rebuild))
