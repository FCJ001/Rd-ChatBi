...
# ★ 多数据源：构造时传入 datasource_id，所有查询按数据源隔离
# ============================================================
...
class PgMetaRepository:
    ...
    def __init__(self, db: AsyncSession, datasource_id: int):
        self.db = db
        self.datasource_id = datasource_id

    async def get_all_tables(self) -> list[TableInfo]:
        result = await self.db.execute(
            select(Nl2sqlTable).where(Nl2sqlTable.datasource_id == self.datasource_id)
        )
        tables = []
    ...
    async def get_all_columns(self) -> list[ColumnInfo]:
        result = await self.db.execute(
            select(Nl2sqlColumn, Nl2sqlTable.table_name).join(
                Nl2sqlTable, Nl2sqlColumn.table_id == Nl2sqlTable.id
            ).where(Nl2sqlTable.datasource_id == self.datasource_id)
        )
        return [_to_column_info(c, tname) for c, tname in result.all()]
    ...
    async def get_table_by_name(self, name: str) -> TableInfo | None:
        result = await self.db.execute(
            select(Nl2sqlTable).where(
                Nl2sqlTable.table_name == name,
                Nl2sqlTable.datasource_id == self.datasource_id,
            )
        )
    ...
    async def get_all_metrics(self) -> list[MetricInfo]:
        result = await self.db.execute(
            select(Nl2sqlMetric).where(Nl2sqlMetric.datasource_id == self.datasource_id)
        )
        return [
            MetricInfo(
    ...
    async def get_metric_by_name(self, name: str) -> MetricInfo | None:
        result = await self.db.execute(
            select(Nl2sqlMetric).where(
                Nl2sqlMetric.metric_name == name,
                Nl2sqlMetric.datasource_id == self.datasource_id,
            )
        )
